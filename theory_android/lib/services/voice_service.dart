...
class VoiceService {
  ...
  /// Maps the user's speed slider (-40..+40) to a flutter_tts rate (0.1..1.0).
  static double _ttsRate() =>
      (0.5 + EdgeTtsService.ratePct / 100).clamp(0.1, 1.0);

  static Future<void> _speakText(String text) async {
    final fixed = _fixPronunciation(text);
    // On Android there is no Edge/Python sidecar, so this returns false and we
    // use the phone's built-in Hebrew voice via flutter_tts.
  ...
  static Future<bool> micAvailable() async {
    // Android/mobile always has a mic; the actual RECORD_AUDIO permission is
    // requested by the STT service's hasPermission. The registry probe below
    // is Windows-only (recording with no capture device crashes there).
