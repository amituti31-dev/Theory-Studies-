...
class VoiceService {
  ...
  static Future<void> init() async {
    ...
    // Wire the Edge engine state FIRST — it is the primary voice on Linux and
    // must be connected even if flutter_tts (no Linux support) throws below.
    EdgeTtsService.onStart = () => _setState(VoiceState.speaking);

    // flutter_tts has no Linux implementation; guard every call so a
    // MissingPluginException never breaks initialization.
    try {
  ...
  /// flutter_tts has no Linux implementation; stopping it must never throw.
  static Future<void> _safeTtsStop() async {
    try {
      await _tts.stop();
    } catch (_) {}
  }
  ...
  static Future<void> _speakText(String text) async {
    ...
    if (!ok) {
      // flutter_tts fallback (unavailable on Linux — guarded).
      try {
